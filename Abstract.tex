...
\begin{figure}[H]
  \begin{center}
   \subfigure[Glia-AgentからNeuro-Agentへの作用:刈り込み]{	% 副題なし
    \includegraphics[width=.5\columnwidth]{FlowChartGN.pdf}
   }~ % 改行
   \subfigure[Neuro-AgentからGlia-Agentへの作用:閾値の調節]{
    \includegraphics[width=.5\columnwidth]{FlowChartNG.pdf}
   }
   \caption{Neuro-Glia相互調節機能の処理フロー} 
   \label{fig:target}
  \end{center}
 \end{figure}
\wfig{target}中の関数$func(Neuron.freq)$は, \weq{new}の関数を用いた
\begin{align}
